    ...
    for product in PRODUCT_KEYWORDS:
        try:
            ...
            if not df.empty:
                ...
                data.append({
                    "Product Name": f"<a href='{google_link}' target='_blank' style='color:#00FFFF;text-decoration:none;font-weight:bold;'>{product.title()}</a>",
                    "Product": product,
                    "Image": f"<a href='{google_link}' target='_blank'><img src='{img_url}' width='60'></a>",
    ...
    children=[
        ...
        dash_table.DataTable(
            ...
            columns=[
                ...
                {"name": "Product Name", "id": "Product Name", "presentation": "markdown"},
        ...
        dcc.Store(id='selected-product-url'),
...
def update_data(n):
    ...
    trend_fig = px.bar(df, x='Product', y='Current Interest', color='Change', title='🔥 Current Search Interest (Last 7 Days)')
...
# Open link when a bar is clicked
@app.callback(
    Output('redirect', 'href'),
    [Input('trend-chart', 'clickData'),
     Input('profit-chart', 'clickData')]
)
def open_link(trend_click, profit_click):
    ctx = dash.callback_context
    if not ctx.triggered:
        return dash.no_update
    click_data = trend_click or profit_click
    if click_data:
